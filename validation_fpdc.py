...
datadir = "D:/MEGAsync/BArbeit/Data/cluster_results/fpdc/"
...
subset = pd.read_csv("D:/MEGAsync/BArbeit/Data/cluster_results/done_fpdc.csv", header=None, sep=",")
...
for subset_entry in subset:
    ...
    nf = int(subset_entry[4])
    nu = int(subset_entry[5])

    cstart = int(subset_entry[6])
    cend = int(subset_entry[7])
    cstep = int(subset_entry[8])

    alt_name = subset_entry[9]
    ...
    for clustercount in clist:
        ...
        config = alt_name + "(k" + str(clustercount) + "nf" + str(nf) + "nu" + str(nu) + ")"
