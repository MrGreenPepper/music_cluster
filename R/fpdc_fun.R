fpdc <- function(t_scaled, algo, nf, nu, cmin, cmax, step_size, dataset){
  ...
  for(nv in cmin:cmax) {
    ...
    save_filename = paste(algo_name, "(k", toString(clusteranzahl), "nf", toString(nf), "nu", toString(nu), ")_", sep="")
    ...
    cluster_data <- FPDC(data = t_scaled, k = clusteranzahl, nf = nf, nu = nu)
...
#    print(cluster_data$probability)
   ...
   try(xb <- XB(t_scaled, cluster_data$probability, cluster_data$centers, 2))
    ...
    filename_x = paste(save_file, "_exp.csv", sep = "")
    write.csv(cluster_data$explained, filename_x, fileEncoding = "UTF-8")
